...
if [ -z "${PERSONAL_ACCESS_TOKEN}" ]; then fatal "PERSONAL_ACCESS_TOKEN must be set before calling"; fi
...
export RUNNER_TOKEN=$(curl -s -X POST ${base_api_url}/${runner_scope}/actions/runners/registration-token -H "accept: application/vnd.github.everest-preview+json" -H "authorization: token ${PERSONAL_ACCESS_TOKEN}" | jq -r '.token')
