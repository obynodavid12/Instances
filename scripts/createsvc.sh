...
sudo -u ubuntu EC2_INSTANCE_ID=`wget -q -O - http://169.254.169.254/latest/meta-data/instance-id` bash -c 'cd /home/ubuntu/actions-runner/;./config.sh --url https://github.com/obynodavid12/Instances  --PERSONAL_ACCESS_TOKEN ${PERSONAL_ACCESS_TOKEN} --name "github-runner"-$${EC2_INSTANCE_ID}" --unattended'
