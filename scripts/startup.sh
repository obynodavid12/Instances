...
sudo -u ubuntu EC2_INSTANCE_ID=`wget -q -O - http://169.254.169.254/latest/meta-data/instance-id` bash -c 'cd /home/ubuntu/actions-runner/;./config.sh --url ${github_repo_url} --pat ${personal_access_token} --name "${runner_name}-$${EC2_INSTANCE_ID}" --runasservice unattended --replace'
